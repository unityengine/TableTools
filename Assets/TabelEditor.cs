...
//				Debug.Log(allLineStr[i]);
		...
		for(int i = 0; i < fieldStrArr.Length;)
		{
			...
			if(vMatchs.Count != 0)
			{
				...
				int index = Convert.ToInt32(System.Text.RegularExpressions.Regex.Replace(fieldStrArr[i], @"[^\d]*", ""));
		...
		{
			int arrMax = TableEditorToServer.ComputeListIntMaxNum(arrDic[k])+1;
			string typeStr = TableEditorToServer.CheckTypeInfomation(k, fieldTypeStrArr, fieldStrArr);

			allCodeText += "\tpublic int get" + k + "Count() { return " + arrMax +"; }\r\n";
			if(typeStr == "string")
			{
				allCodeText += "\tprivate string[] m_" + k + " = new string[" + arrMax + "];\r\n";
				allCodeText += "\tpublic string Get" + k + @"byIndex(int idx)
				{
					if (idx >= 0 && idx < " + arrMax + ") return m_" + k + "[idx];return \"\";}";
			}else if(typeStr == "bool")
			{
				allCodeText += "\tprivate bool[] m_" + k + " = new bool[" + arrMax + "];\r\n";
				allCodeText += "\tpublic bool Get" + k + @"byIndex(int idx)
				{
					if (idx >= 0 && idx < " + arrMax + ") return m_" + k + @"[idx];
					return false;
				}";
			}else if(typeStr == "double")
			{
				allCodeText += "\tprivate double[] m_" + k + " = new double[" + arrMax + "];\r\n";
			...
			{
				allCodeText += "\tprivate float[] m_" + k + " = new float[" + arrMax + "];\r\n";
			...
			{
				allCodeText += "\tprivate int[] m_" + k + " = new int[" + arrMax + "];\r\n";
			...
			{
				allCodeText += "\tprivate short[] m_" + k + " = new short[" + arrMax + "];\r\n";
			...
			{
				allCodeText += "\tprivate byte[] m_" + k + " = new byte[" + arrMax + "];\r\n";
		...
		for(int i = 0; i < fieldStrArr.Length; i++)
		{
			...
			if(vMatchs.Count != 0)
			{
				...
				{
					allCodeText +=  "\t_values.m_" + loField + "[" + index.ToString() + "] = valuesList[(int)_ID.ID_" + fieldStrArr[i].ToUpper() + "] as string; \r\n";
				}else if(fieldTypeStrArr[i] == "bool")
				{
					allCodeText +=  "\t_values.m_" + loField + "[" + index.ToString() + "] = Convert.ToInt16(valuesList[(int)_ID.ID_" + fieldStrArr[i].ToUpper() + "] as string) != 0; \r\n";
				}else if(fieldTypeStrArr[i] == "double")
				{
					allCodeText +=  "\t_values.m_" + loField + "[" + index.ToString() + "] = Convert.ToDouble(valuesList[(int)_ID.ID_" + fieldStrArr[i].ToUpper() + "] as string); \r\n";
				}else if(fieldTypeStrArr[i] == "float")
				{
					allCodeText +=  "\t_values.m_" + loField + "[" + index.ToString() + "] = Convert.ToSingle(valuesList[(int)_ID.ID_" + fieldStrArr[i].ToUpper() + "] as string); \r\n";
				}else if(fieldTypeStrArr[i] == "int")
				{
					allCodeText +=  "\t_values.m_" + loField + "[" + index.ToString() + "] = Convert.ToInt32(valuesList[(int)_ID.ID_" + fieldStrArr[i].ToUpper() + "] as string); \r\n";
				}else if(fieldTypeStrArr[i] == "short")
				{
					allCodeText +=  "\t_values.m_" + loField + "[" + index.ToString() + "] = Convert.ToInt16(valuesList[(int)_ID.ID_" + fieldStrArr[i].ToUpper() + "] as string); \r\n";
